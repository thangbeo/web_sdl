<template>
  <v-col class="py-0" cols="11">
    ...
    <div>
      <v-radio-group v-model="type" row @change="set_data_change">
    ...
    <div v-if="type === 3">
      ...
      >
        ...
        <template v-slot:item="data">
          <tr v-if="open_edit !== data.item.id">
            <td>
              {{ set_day_value(data.item.time[0]) }}
            </td>
            <td>
              {{ set_day_value(data.item.time[1]) }}
            </td>
            <td>
              {{
                `${$formatMoney({ amount: data.item.adult_price })} ${
                  data.item.currency.text
                }`
              }}
            </td>
            <td>
              {{
                `${$formatMoney({ amount: data.item.chil_price })} ${
                  data.item.currency.text
                }`
              }}
            </td>
          ...
          <tr v-else>
            <td>
              ...
              >
                <template v-slot:activator="{ on }">
                  <v-text-field
                    :value="set_day_value(data.item.time[0])"
                    class="border-1"
                    hide-details
                    dense
                    flat
            ...
            <td>
              ...
              >
                <template v-slot:activator="{ on }">
                  <v-text-field
                    :value="set_day_value(data.item.time[1])"
                    class="border-1"
                    hide-details
                    dense
                    flat
                ...
                <v-menu top disabled>
                  ...
                  <v-list dense>
                    <v-list-item
                      ...
                      :key="`${i}-${money.value}`"
                      @click="set_currency(money, data.item)"
                    >
...
import Add from '~/components/product-nha-cung-cap/tour/DepartureSchedule/add'
