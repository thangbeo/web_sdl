    ...
    <v-row v-if="hydrated">
      <v-col cols="12" class="d-flex sticky py-0" style="top: 75px; z-index: 2">
        ...
        <v-btn
          ...
          depressed
          width="120px"
          :disabled="$wait.is('loadingPreview')"
          @click="getUrlPreview"
        >
      ...
      >
        ...
        <template v-else-if="sysField.id === 14">
            ...
            <template v-for="(tab, tabIdx) in sysField.attributeLangList">
              ...
              >
                <v-text-field
                  ...
                  flat
                  dense
                  :prefix="$store.state.app.domain_name"
                  :readonly="readonly"
                  @input="tab.errors = []"
                >
                  <template v-slot:append>
                    <v-tooltip bottom>
                      <template v-slot:activator="{ on, attrs }">
                        <v-btn
                          class="rounded-l-0 mr-n3"
                          style="width: 36px; min-width: 36px"
                          color="primary"
                          depressed
                          v-bind="attrs"
                          v-on="on"
                          @click="copy_text(`copy-text-${tabIdx}`)"
                        >
                ...
                <div :id="`copy-text-${tabIdx}`" class="d-none">
                  {{ `${$store.state.app.domain_name}${tab.value}` }}
                </div>
              </v-tab-item>
            </template>
          </v-tabs>
      ...
      <v-footer v-if="!readonly" inset app>
        <v-row>
          <v-spacer></v-spacer>
          <v-btn
            class="text-none white--text mr-3"
...
import Suggestions from '~/components/Product/edit/hotel/suggestions/index.vue'
import CopyProduct from '~/components/Product/copy'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      markerLatLng: [21.028511, 105.804817],
      lat: 21.028511,
      lng: 105.804817,
      zoom: 15,
      address: '',
  ...
  methods: {
          ...
          if (cur.id === 6) {
            ...
            for (
              ...
              if (
                ...
                this.$log.debug('[textFieldtextArea]', data)
                const attributeLangValue = data.length
                  ? data[0].values.filter((e) => e.langId === lang.id)[0]
                  : undefined
                attributeLang.value =
                  data.length && attributeLangValue
                    ? attributeLangValue.value[0]
                    : null
                ...
                attributeLang.value =
                  data.length && attributeLangValue
                    ? isNaN(attributeLangValue.value[0])
                      ? attributeLangValue.value[0]
                      : Number(attributeLangValue.value[0])
                    : null
                ...
                attributeLang.value =
                  data.length && attributeLangValue
                    ? attributeLangValue.value.reduce(
                        (dataAcc, curData, dataIdx) => {
                          dataAcc.push(
                            isNaN(curData) ? curData : Number(curData)
                          )
                          return dataAcc
                        },
                        []
          ...
          if (dataField.id === 6) {
            ...
            for (
              ...
              if (
                ...
                if (isRequired) {
                  this.$log.debug('[textFieldtextAreadate]', lang)
                      ...
                      .reduce((attrAcc, curAttr, attrIdx) => {
                        if (curField.attributeTypeId === attrTypes.dateTime) {
                          ...
                          attrAcc.push({
                            attributeId: curField.id,
                            value:
                              curAttr.value && curAttr.value.length === 0
                                ? ''
                                : curAttr.value || '',
                            langId: curLangId,
                          })
    ...
    // start tuan.pham
    async getUrlPreview() {
      this.$wait.start('loadingPreview')
      const attrTypes = { ...this.$store.state.productAttrs.attrTypes }
      const filesToUpload = [
        ...this.systemFields
          .filter(
      ...
      for (let index = 0; index < filesToUpload.length; index++) {
        ...
        try {
          const uploadResult = await this.uploadFile(element)
          if (!uploadResult.error) {
            this.$log.debug('[Upload Result]', uploadResult)
            element.value = uploadResult.data.data.image_url
            success++
          }
        } catch (error) {
      ...
      if (success === filesToUpload.length) {
        ...
        this.$log.debug('[Request Data]', data)
        this.$store
          .dispatch('tourismProduct/getUrlPreview', data)
          .then((response) => {
            this.$log.debug('[Response Data]', response)
            if (!response.error) {
    ...
    copy_product(name) {
      const DATA = {
        ...
        product_type: this.$store.state.app.product_type.products,
        product_name: name,
      }
      this.$store
