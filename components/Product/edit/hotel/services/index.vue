...
<script>
import Add from '~/components/Product/edit/hotel/services/add'
import Edit from '~/components/Product/edit/hotel/services/edit'
...
export default {
  ...
  created() {
    ...
    for (let i = 0; i < arr.length; i++) {
      for (let j = 0; j < arr[i].convenientServices.length; j++) {
        arr[i].convenientServices[j] = {
          id: j + 1,
          ...arr[i].convenientServices[j],
        }
      }
      arr[i].item = [...arr[i].convenientServices]
    }
    this.utilities = [...arr]
  ...
  methods: {
    get_data() {
      this.$store.dispatch('product/hotel_convenient_services').then((res) => {
        if (!res.error) {
          let arr = []
