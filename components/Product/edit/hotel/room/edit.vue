<template>
  <div>
    <dialog-content-full :open="open">
      <div class="d-flex">
        <div class="fs-218 ml-4 d-flex align-center font-weight-bold">
    ...
    open(value) {
      if (value) {
        ...
        for (let i = 0; i < this.moneys.length; i++) {
          if (this.moneys[i].value === this.data.currency_id) {
            this.currency = {
              value: this.data.currency_id,
              text: this.moneys[i].text,
            }
          }
        }

        this.$store.dispatch('product/hotel_room_type').then((res) => {
          if (!res.error) {
            const arr = []
        ...
        this.$store.dispatch('product/hotel_bed_type').then((res) => {
          if (!res.error) {
            const arr = []
            if ((res.data.data || []).length !== 0) {
              for (let i = 0; i < res.data.data.length; i++) {
        ...
        this.type_room = {
          value: this.data.room_type_id,
          text: this.data.room_type_name,
        }
        this.type_bed = this.data.bed_type
        this.status = this.data.status
        this.acreage = this.data.acreage
        this.capacity_adults = this.data.number_adults
        this.capacity_child = this.data.number_children
        this.room = this.data.number_rooms
        this.price = this.data.price_value
      } else {
        this.reset()
      }
    },
    ...
    save() {
      ...
      if (!hasErrors) {
        const DATA = {
          ...
          id: this.data.id,
          enable: this.hidden,
          type_id: this.type_room.value,
          type_name: this.type_room.text,
          ...
          room_number: parseInt(this.room),
        ...
        this.$store.dispatch('product/room_edit', DATA).then((res) => {
          if (!res.error) {
            const data = {
              ...
              number_adults: this.capacity_adults,
              number_children: this.capacity_child,
              number_rooms: this.room,
