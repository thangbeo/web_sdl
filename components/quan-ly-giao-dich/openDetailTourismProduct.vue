...
export default {
  ...
  methods: {
    ...
    change_status(value) {
      if (!this.$isNullOrEmpty(this.tourism.master.order_status)) {
        const DATA = {
          items: [
            {
    ...
    get_list() {
      this.items = this.tourism.change_history
      this.transactionProducts = this.tourism.transactionProducts
      this.productDetail = this.tourism.master
      this.productsInfo = this.tourism.productsInfo
      this.status = this.tourism.master.order_status
      if (this.status === 0) {
