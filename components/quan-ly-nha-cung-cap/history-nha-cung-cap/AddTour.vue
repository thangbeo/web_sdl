  ...
  <div>
    <div>
      ...
      <v-row class="pl-8">
        ...
        <v-col v-if="day_type === 1" cols="9" class="py-0">
          <v-menu
            ref="menu1"
            v-model="menuDate"
        ...
        <v-col v-if="day_type === 2" cols="9" class="py-0">
          <v-menu
            ref="menu2"
            v-model="menuDate2"
            :close-on-content-click="false"
        ...
        <v-col v-if="day_type === 3" cols="9" class="py-0">
          <v-select
            v-model="startDay"
            :items="list_startDay"
      ...
      <v-row v-if="day_type === 1" class="pl-8">
        <v-col cols="2" class="pl-10 py-0">
          <v-card-text class="text-subtitle-2 pt-2">Giá người lớn </v-card-text>
        </v-col>
        <v-col cols="9" class="py-0">
          <v-row>
            <v-col cols="4" class="pt-0 ml-1">
              <v-card-text class="pl-0 pt-2 font-weight-bold">
                {{ `${$formatMoney({ amount: priceAdultDaily })}` }}
                VND
              </v-card-text>
            </v-col>
            <v-col cols="2" class="pt-0">
              <v-card-text class="text-subtitle-2 pt-2">Giá trẻ em</v-card-text>
            </v-col>
            <v-col cols="4" class="pt-0 pl-0">
              <v-card-text class="pl-0 pt-2 font-weight-bold"
                >{{
                  `${$formatMoney({ amount: priceChildDaily })}`
                }}
                VND</v-card-text
              >
            </v-col>
      ...
      <v-row v-if="day_type === 2" class="pl-8">
        ...
        <v-col cols="9" class="py-0">
          <v-row>
            <v-col cols="4" class="pt-0 ml-1">
              <v-card-text class="pl-0 pt-2 font-weight-bold">
                {{ `${$formatMoney({ amount: priceAdultWeekly })}` }}
              </v-card-text>
            </v-col>
            <v-col cols="2" class="pt-0">
              <v-card-text class="text-subtitle-2 pt-2">Giá trẻ em</v-card-text>
            ...
            <v-col cols="4" class="pt-0 pl-0">
              <v-card-text class="pl-0 pt-2 font-weight-bold"
                >{{
                  `${$formatMoney({ amount: priceChildWeekly })}`
                }}
                VND</v-card-text
              >
            </v-col>
      ...
      <v-row class="pl-8 mt-6">
        <v-col cols="2" class="pl-10 py-0">
          <v-card-text class="text-subtitle-2 pt-3">Tổng số </v-card-text>
        </v-col>
        <v-col cols="9" class="py-0 ml-1">
          <v-card-text class="pl-0 pt-3 font-weight-bold"
            >{{ `${$formatMoney({ amount: total_price })}` }} VND</v-card-text
          >
        </v-col>
      </v-row>
      <v-row class="pl-8 pt-3">
        <v-col cols="2" class="pl-10 py-0 pr-2">
          <v-card-text class="text-subtitle-2 pt-3"
            >Đơn vị thanh toán</v-card-text
          >
        </v-col>
        <v-col cols="9" class="py-0 pr-2">
          <v-select
            v-model="currency"
      ...
      <v-row class="pl-8 mb-8">
        <v-col cols="2" class="pl-10 py-0">
          <v-card-text class="text-subtitle-2 pb-3">Thanh toán</v-card-text>
        </v-col>
        <v-col cols="9" class="py-0 ml-1">
          <v-card-text
            v-if="currency !== list_currencies[0] && paymentPrice > 0"
            class="pl-0 pb-3 font-weight-bold"
            ><span
              >{{
                $formatMoney({
          ...
          <v-card-text
            ...
            class="pl-0 font-weight-bold pb-3"
            ><span
              >{{
  ...
  methods: {
    ...
    Add() {
      ...
      this.$store
        ...
        .then((response) => {
          if (!response.error) {
            this.$showSuccess(this, 'Thêm giao dịch đặt tour thành công')
            this.$emit('exit')
          }
        })
        .catch((e) => {
      ...
      this.$store.dispatch('giao_dich/list_shedule_tour', data).then((res) => {
        if (!res.error) {
          if (!this.$isNullOrEmpty(res.data.data)) {
            ...
            this.priceAdultDaily =
              res.data.data.tourDepartureScheduleDaily.adult_price
            this.priceChildDaily =
              res.data.data.tourDepartureScheduleDaily.child_price
            this.priceAdultWeekly =
              res.data.data.tourDepartureScheduleWeekly.adult_price
            this.priceChildWeekly =
              res.data.data.tourDepartureScheduleWeekly.child_price
            ...
            const arr = []
            if (
              (res.data.data.tourDepartureScheduleCustoms || []).length !== 0
            ) {
              for (
                let i = 0;
      ...
      this.$store.dispatch('giao_dich/price_tour', data).then((res) => {
        if (!res.error) {
          if (!this.$isNullOrEmpty(res.data.data)) {
            ...
            this.priceAdult = res.data.data.adult_price
            this.priceChild = res.data.data.chil_price
            this.total_tour_price()
          }
        }
      })
